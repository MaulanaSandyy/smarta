    ...
    <div x-data="dataKalenderLingkungan()">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 mb-6">
            <div class="flex items-center gap-3">
                <button class="btn-ghost btn-sm p-1.5" @click="prevMonth()" title="Bulan sebelumnya">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                </button>
                <h2 class="text-lg font-semibold text-text-primary min-w-[180px] text-center" x-text="namaBulan[bulan - 1] + ' ' + tahun"></h2>
                <button class="btn-ghost btn-sm p-1.5" @click="nextMonth()" title="Bulan berikutnya">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                </button>
            </div>
            <button class="btn-primary btn-sm justify-center" @click="bukaForm()">
        ...
        <div class="bg-(--surface) border border-border rounded-2xl shadow-sm overflow-hidden">
            <div class="grid grid-cols-7 gap-px bg-border">
                ...
                <template x-for="(day, i) in calendarGrid" :key="i">
                    <div class="bg-(--surface) min-h-[90px] sm:min-h-[110px] p-1.5 sm:p-2 hover:bg-surface-secondary transition-colors relative"
                         :class="day !== null && isToday(day) ? 'ring-2 ring-primary-500 ring-inset' : ''">
                        <template x-if="day !== null">
                            <div>
                                ...
                                <div class="mt-1 space-y-0.5">
                                    <template x-for="ev in eventsForDay(day)" :key="ev.id">
                                        <div class="flex items-center gap-1 group">
                                            <span class="inline-block w-1.5 h-1.5 rounded-full shrink-0" :class="warnaKategori(ev.kategori)"></span>
                                            <span class="text-[10px] text-text-primary truncate leading-tight" x-text="ev.judul"></span>
                                            <button class="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity text-rose-500 hover:text-rose-700" @click="hapusAgenda(ev.id)" title="Hapus">
        ...
        <div class="mt-6" x-show="upcomingEvents.length > 0">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <template x-for="ev in upcomingEvents" :key="ev.id">
                    <div class="bg-(--surface) border border-border rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow">

<x-layouts.rt>
    ...
    <x-slot:subtitle>Catat pemasukan atau pengeluaran kas RT</x-slot:subtitle>

    <div class="max-w-2xl">
        <x-ui.card>
            <form action="{{ route('rt.kas.store') }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <select name="warga_id" class="input w-full">
                        ...
                        @foreach($warga as $w)
                        <option value="{{ $w->id }}" @selected(old('warga_id') == $w->id)>{{ $w->nama }} - {{ $w->nik }}</option>
                        @endforeach
                    </select>
                    @error('warga_id')
                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" name="kategori" class="input w-full" value="{{ old('kategori') }}">
                    @error('kategori')
                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" class="input w-full" value="{{ old('tanggal', date('Y-m-d')) }}">
                    @error('tanggal')
                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="bukti" class="input w-full" value="{{ old('bukti') }}" placeholder="Link/file bukti transaksi">
                    @error('bukti')
                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>

<x-layouts.rt>
    ...
    <div x-data="dataKas({{ Js::from($transaksi) }}, {{ $perPage }})">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="text-3xl font-bold text-text-primary" x-text="formatRupiah(saldo)"></p>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                    <p class="text-sm text-text-muted mb-1">Total Pemasukan</p>
                    <p class="text-3xl font-bold text-emerald-600" x-text="formatRupiah(totalMasuk)"></p>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="text-3xl font-bold text-rose-600" x-text="formatRupiah(totalKeluar)"></p>
                    <p class="text-xs text-text-muted mt-1">Bulan Mei 2026</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-(--surface) border border-border rounded-2xl p-4 shadow-sm mb-6">
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                <div class="relative flex-1 max-w-xs">
                    ...
                    <input type="text" placeholder="Cari keterangan..." class="input pl-10" x-model="search" @input="page = 1">
                </div>
                <div class="flex items-center gap-2 flex-wrap">
                    {{-- Type filter --}}
                    <div class="flex items-center gap-1.5">
                        <button @click="setFilter('typeFilter', '')" class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors" :class="typeFilter === '' ? 'bg-primary-600 text-white shadow-sm' : 'bg-surface-secondary text-text-secondary hover:bg-surface-tertiary'">Semua</button>
                    ...
                    <template x-for="k in kategoriList" :key="k">
                        <button @click="setFilter('kategoriFilter', kategoriFilter === k ? '' : k)" class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors" :class="kategoriFilter === k ? 'bg-primary-600 text-white shadow-sm' : 'bg-surface-secondary text-text-secondary hover:bg-surface-tertiary'" x-text="k"></button>
                    </template>
            ...
            <template x-for="(item, index) in pagedItems" :key="index">
                <div class="bg-(--surface) border border-border rounded-2xl p-4 shadow-sm">
                    <div class="flex items-start justify-between mb-2">
                        ...
                        <span class="badge-slate text-[10px] px-2 py-0.5 flex-shrink-0" x-text="item.kategori"></span>
                    </div>
                            ...
                            <tr class="hover:bg-surface-secondary transition-colors">
                                ...
                                <td class="table-cell">
                                    <template x-if="item.bukti">
                                        <button class="text-primary-600 hover:text-primary-700 text-sm font-medium" @click="detail(item)">Lihat</button>
                                    </template>
                                    <template x-if="!item.bukti">
        ...
        <x-ui.modal name="detail-kas" header="Detail Transaksi">

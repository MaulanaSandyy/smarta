<x-layouts.rt>
    <x-slot:title>Buat Informasi</x-slot:title>
    <x-slot:subtitle>Bagikan informasi ke warga</x-slot:subtitle>

    <div class="max-w-2xl">
        <x-ui.card>
            <form action="{{ route('rt.informasi.store') }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="isi" class="input w-full" rows="6">{{ old('isi') }}</textarea>
                    @error('isi')
                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="penting" value="1" class="rounded" @checked(old('penting'))>
                        <span>Penting</span>
                    </label>
                    @error('penting')
                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="diterbitkan" value="1" class="rounded" @checked(old('diterbitkan', true))>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_terbit" class="input w-full" value="{{ old('tanggal_terbit', date('Y-m-d')) }}">

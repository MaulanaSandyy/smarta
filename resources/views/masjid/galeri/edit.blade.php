<x-layouts.masjid>
    <x-slot:title>Edit Galeri</x-slot:title>
    <x-slot:subtitle>Ubah konten galeri</x-slot:subtitle>

    <div class="max-w-2xl">
        <x-ui.card>
            <form action="{{ route('masjid.galeri.update', $galeri) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <input type="hidden" name="user_id" value="{{ auth()->id() }}">
                <div>
                    <label class="block text-sm font-medium text-text-primary mb-1">Judul <span class="text-rose-500">*</span></label>
                    <input type="text" name="judul" class="input w-full" value="{{ old('judul', $galeri->judul) }}" maxlength="255" required>
                ...
                <div>
                    ...
                    <select name="tipe" class="input w-full" required>
                        ...
                        <option value="video" @selected(old('tipe', $galeri->tipe) == 'video')>Video</option>
                        <option value="artikel" @selected(old('tipe', $galeri->tipe) == 'artikel')>Artikel</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="url" name="url" class="input w-full" value="{{ old('url', $galeri->url) }}" placeholder="Link untuk video/artikel">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" class="input w-full" rows="4">{{ old('deskripsi', $galeri->deskripsi) }}</textarea>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" class="input w-full" value="{{ old('tanggal', $galeri->tanggal) }}" required>
                ...
                <div>
                    ...
                    <select name="status" class="input w-full" required>
                        <option value="">Pilih...</option>
                        <option value="draft" @selected(old('status', $galeri->status) == 'draft')>Draft</option>
                        <option value="terbit" @selected(old('status', $galeri->status) == 'terbit')>Terbit</option>
                        <option value="arsip" @selected(old('status', $galeri->status) == 'arsip')>Arsip</option>
                    </select>

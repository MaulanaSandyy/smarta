export default function dataKeuangan(items, perPage) {
    return {
        page: 1,
        perPage: perPage,
        search: '',
        ...
        get filteredItems() {
            ...
            return self.items.filter(function(item) {
                ...
                var matchSearch = !self.search || item.ket.toLowerCase().includes(q) || item.kategori.toLowerCase().includes(q);
        ...
        detail: function(item) {
            ...
            if (window['modal-detail-keuangan']) {
                window['modal-detail-keuangan'].open();
            }
        },
        bukaForm: function() {
            var now = new Date();
            var today = String(now.getDate()).padStart(2, '0') + '/' + String(now.getMonth() + 1).padStart(2, '0') + '/' + now.getFullYear();
            this.form = { tgl: today, ket: '', kategori: 'Infaq', masuk: 0, keluar: 0 };
            if (window['modal-tambah-keuangan']) {
                window['modal-tambah-keuangan'].open();
            }
        },
        tambahTransaksi: function() {
            if (!this.form.ket || (!this.form.masuk && !this.form.keluar)) return;
            this.items.push({
                tgl: this.form.tgl,
                ket: this.form.ket,
                kategori: this.form.kategori,
                masuk: parseInt(this.form.masuk) || 0,
                keluar: parseInt(this.form.keluar) || 0,
                bukti: false,
            });
            this.form = { tgl: '', ket: '', kategori: 'Infaq', masuk: 0, keluar: 0 };
            if (window['modal-tambah-keuangan']) {
                window['modal-tambah-keuangan'].close();
            }
        },
        generateTableHTML: function(includePrintStyles) {
            ...
            var h = '<table border=\"1\" cellpadding=\"6\" cellspacing=\"0\" style=\"border-collapse:collapse;width:100%;font-family:Arial,sans-serif;font-size:12px\"><thead><tr style=\"background:#059669;color:white\">' + th('Tanggal') + th('Keterangan') + th('Kategori') + th('Pemasukan') + th('Pengeluaran') + '<\/tr><\/thead><tbody>';
            ...
            var f = '<\/tbody><\/table>';
            var t = '<h2 style=\"font-family:Arial,sans-serif;margin-bottom:16px\">Keuangan Masjid Al-Barakah<\/h2>';
        ...
        printPDF: function() {
            ...
            var doc = '<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>Keuangan Masjid<\/title><style>' + style + '<\/style><\/head><body>' + content + '<\/body><\/html>';
